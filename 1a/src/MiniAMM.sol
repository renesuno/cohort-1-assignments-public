...
contract MiniAMM is IMiniAMM, IMiniAMMEvents {
    ...
    // MiniAMM 컨트랙트 생성자, // 두 토큰 주소를 받아 풀을 초기화, 실제 구현 필요
    ...
    function _addLiquidityFirstTime(
        ...
        k = xReserve * yReserve;
    } // 최초 유동성 추가 시 호출되는 내부 함수, // 초기 k 값 결정 및 xReserve, yReserve 설정
    ...
    function _addLiquidityNotFirstTime(
        ...
        k = xReserve * yReserve;
    } // 이미 유동성이 있는 경우 추가 유동성 처리, // 기존 k 값을 유지하면서 xReserve, yReserve 증가
